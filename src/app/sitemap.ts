...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    { url: `${baseUrl}/landing`, lastModified: now, changeFrequency: "monthly", priority: 1.0 },
  ...
  const featurePages: MetadataRoute.Sitemap = featureSlugs.map(slug => ({
    url: `${baseUrl}/features/${slug}`, lastModified: now, changeFrequency: "monthly" as const, priority: 0.7,
  }));

  // Showcase pages
  const showcaseSlugs = ['learning', 'fitness', 'content', 'productivity', 'analytics', 'gamification'];
  const showcasePages: MetadataRoute.Sitemap = showcaseSlugs.map(slug => ({
    url: `${baseUrl}/showcase/${slug}`, lastModified: now, changeFrequency: "monthly" as const, priority: 0.6,
  }));

  // Dynamic blog posts — use dynamic import to avoid Prisma init failure at build time
  ...
  try {
    ...
    // If DB query fails (e.g. at build time without DATABASE_URL), skip blog pages
  }
