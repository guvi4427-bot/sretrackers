...
export async function POST(
  ...
  try {
    ...
    const existing = await db.liveUpdateLike.findUnique({
    ...
    const like = await db.liveUpdateLike.create({
      data: { entityType, entityId: updateId, userId: session.user.id },
    });

    return NextResponse.json({ liked: true, id: like.id });
  } catch (error) {
    console.error('POST /api/live-updates/[updateId]/like error:', error);
...
export async function DELETE(
  ...
  try {
    ...
    await db.liveUpdateLike.deleteMany({
      where: { entityType, entityId: updateId, userId: session.user.id },
    });

    return NextResponse.json({ liked: false });
  } catch (error) {
    console.error('DELETE /api/live-updates/[updateId]/like error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
